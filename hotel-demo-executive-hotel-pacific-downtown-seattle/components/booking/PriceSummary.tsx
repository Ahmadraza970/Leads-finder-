...
import { formatINR, getRoom } from "@/lib/booking";
      ...
      <dl className="mt-6 space-y-3 border-t border-beige pt-5 text-sm">
        <div className="flex justify-between gap-4">
          <dt className="text-soft">
            Room rate ({price.nights} × {formatINR(price.ratePerNight)})
          </dt>
          <dd className="font-semibold text-dark">{formatINR(price.roomSubtotal)}</dd>
        </div>
        <div className="flex justify-between gap-4">
          <dt className="text-soft">GST @ {Math.round(price.gstRate * 100)}%</dt>
          <dd className="font-semibold text-dark">{formatINR(price.gst)}</dd>
        </div>
